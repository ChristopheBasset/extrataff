...
export default function Register() {
  ...
  const userType = searchParams.get('type') // 'talent', 'establishment' ou 'express'
  const isExpress = userType === 'express'
  ...
  useEffect(() => {
    if (!userType || (userType !== 'talent' && userType !== 'establishment' && userType !== 'express')) {
      navigate('/')
    }
  }, [userType, navigate])
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      sessionStorage.setItem('registration_type', isExpress ? 'establishment' : userType)
      sessionStorage.setItem('registration_success', 'true')
      if (isExpress) sessionStorage.setItem('express_flow', '1')
      ...
      <div className="max-w-md mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="bg-white rounded-xl shadow-lg p-8">
          ...
          <div className="text-center mb-8">
            <div className="text-5xl mb-3">{isExpress ? '⚡' : userType === 'talent' ? '👤' : '🏪'}</div>
            <h2 className="text-2xl font-bold text-gray-900 mb-2">
              {isExpress ? 'Recruter en Express' : `Créer mon compte ${userType === 'talent' ? 'talent' : 'établissement'}`}
            </h2>
            <p className="text-gray-600 text-sm">
              {isExpress ? 'Étape 1/2 : créez votre accès, puis postez votre mission urgente' : 'Étape 1 : Créez votre compte'}
            </p>
          </div>
